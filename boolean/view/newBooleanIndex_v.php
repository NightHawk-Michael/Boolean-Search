...
//		var disabled_type = [ "sd_evs", "sd_int", "sd_trm","sd_rsm","sd_ssm","dd_ang","dd_edm","dd_gpv","dd_lev","dd_sar","dd_str","fd_ele","fd_gra","fd_mag","fd_mgv","gd","gd_sol","td","med"];	
	...
	//	var file_to_submit = "../booleanSubmit.php";
		...
		var build_all_data = function() {
			var list = ["volcano","eruption","monitoring"];
			for (var i = 0; i<list.length; i++) {
				$("div[id='"+list[i]+" Criteria']").find("option ").each(function() {
					$("[name^='"+list[i]+"']").append( build_data( $(this).val(), $(this).text() ) ); 
					// console.log($("#"+list[i]).append( build_data( $(this).val(), $(this).text() ) ));
				});
				//$("select[name^='"+list[i]+"']").append( build_data( $(this).val(), $(this).text() ) );
			}
		...
		var build_data = function(val,text) {
			...
			console.log($.inArray(val,has_dropdownbox));
			if ( $.inArray(val,has_dropdownbox)!=-1 ) {
		 ...
		 -->
				<div id="Volcano Criteria">
					...
					<select name="feature[]" multiple="multiple">
						<option value="feature" disabled selected="">Features</option>
					</select>
					<br>
					<label>Rock Types: </label>
					<select name="rock[]" multiple="multiple" ">
						<option value="rock" disabled selected="">Rock Types</option>
					</select>
					<br>
					<br>
				</div>

				<div id="Eruption Criteria">
				...
				<div id="Monitoring Criteria">
					...
					<!-- <select name="Eruption Search" multiple="multiple"></select>
					<br>
					<optgroup label="Eruption Criteria">
						<option value="edPhase">Eruption Phases</option>
						<option value="vei">VEI</option>
